export class JSRenderedPageError extends Error {
  constructor() {
      ...
      for (const entry of entries) {
        ...
        if (entry['@type'] === 'HowTo' && Array.isArray(entry.step)) {
          for (const step of entry.step) {
            // HowToSection: WikiHow and others nest steps inside sections
            if (Array.isArray(step.itemListElement)) {
              for (const substep of step.itemListElement) {
                const text = substep.text ?? substep.name;
                if (text && typeof text === 'string' && text.length > 2) items.push(text.slice(0, 300));
              }
            } else {
              const text = step.text ?? step.name;
              if (text && typeof text === 'string') items.push(text.slice(0, 300));
            }
          }
        }
...
function isValidListItem(text: string): boolean {
  ...
  // Filter footnote back-references (Wikipedia/WikiHow ↑ style)
  if (text.startsWith('↑')) return false;

  // Filter user comment entries: "Name Month DD, YYYY ..." pattern
  if (/^[A-Za-z\s]+\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)\b\s+\d{1,2},?\s+\d{4}\b/.test(text)) return false;

  // Filter truncated comment links ("..." more) and review ratings
  if (/["']\s*\.\.\.\s*["']\s*more/i.test(text)) return false;
  if (/Rated this article:/i.test(text)) return false;
...
export function extractListsFromHTML(html: string, url: string): string[] {
